  ...
  return <div className={`app-shell theme-${colorTheme} ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <main className="dashboard">
      ...
      {isAdministrator() && view === 'users' && <section className="users-view" aria-busy={usersLoading}>
        ...
        {!usersLoading && usersError && <section className="report-panel users-error-state" role="alert"><span className="users-error-icon" aria-hidden="true">!</span><div><p className="section-kicker">Ação não concluída</p><h2>Não foi possível atualizar os acessos</h2><p className="users-error-message">{usersError}</p><p className="users-error-guidance"><b>O que fazer agora:</b> atualize a lista e tente a ação novamente. Se o problema continuar, fale com o administrador responsável pelo sistema.</p></div><button className="retry" onClick={() => void loadUsers()}>Atualizar lista</button></section>}
